...
class Conv2d(Model):
    ...
    def build(self, in_channels, out_channels,  # pylint: disable=too-many-arguments
        ...
        w = np.zeros((out_channels, in_channels // groups, kernel_size[0], kernel_size[1]),
                     dtype="float32")
    ...
    @script
    def forward(self, x):
        x = mnm.conv2d(x, self.w, stride=self.stride, padding=self.padding,
...
class BatchNorm(Model):
    ...
    def build(self, num_features, eps=1e-5, momentum=0.1, affine=True):
        ...
        self.running_mean = Parameter(np.zeros(num_features, dtype="float32"), name="running_mean")
        self.running_var = Parameter(np.ones(num_features, dtype="float32"), name="running_var")
    ...
    @script
    def forward(self, x):
        ret = mnm.batch_norm_train(
            x=x, w=self.w, b=self.b,
