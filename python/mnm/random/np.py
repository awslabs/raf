...
def uniform(low=0.0, high=1.0, shape=None, name="", device='cpu', dtype='float32'): # pylint: disable=too-many-arguments
    return _wrap(np.random.uniform(low=low, high=high, size=shape), name, dtype, device)


def normal(mean=0.0, std=1.0, shape=None, name="", device='cpu', dtype='float32'): # pylint: disable=too-many-arguments
    return _wrap(np.random.normal(loc=mean, scale=std, size=shape), name, dtype, device)


def zeros_(shape=None, name="", device='cpu', dtype='float32'): # pylint: disable=too-many-arguments
