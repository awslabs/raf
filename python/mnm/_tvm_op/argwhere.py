...
@_tvm.target.override_native_generic_func("mnm_argwhere_strategy")
def mnm_argwhere_strategy(attrs, inputs, out_type, target):  # pylint: disable=unused-argument
...
@mnm_argwhere_strategy.register(["cuda", "gpu"])
def mnm_argwhere_strategy_cuda(attrs, inputs, out_type, target):  # pylint: disable=unused-argument
    ...
    strategy.add_implementation(
        ...
        name="mnm.argwhere.cuda",
    )
    return strategy


_reg.register_strategy("mnm.op.tvm.upper_bound.argwhere", mnm_argwhere_strategy)
