...
void Conv2dCUDNN::Execute(Array<Value> args, Attrs) {
  ...
  CUDNN_CALL(cudnnConvolutionForward(handle, alpha, in_desc, in->data, ker_desc, fil->data, conv_desc, algo, ws, ws_size, beta, out_desc, out_data));
}
...
void Conv2dCUDNN::PreAllocate(Array<Value> args, Attrs attrs) {
  ...
  CUDNN_CALL(cudnnSetTensorNdDescriptor(in_desc, in_tensor.dt, in_tensor.n, in_tensor.dims, in_tensor.strides));
  ...
  CUDNN_CALL(cudnnSetFilterNdDescriptor(ker_desc, ker_tensor.dt, CUDNN_TENSOR_NCHW, ker_tensor.n, ker_tensor.dims));
  ...
  CUDNN_CALL(cudnnGetConvolutionNdForwardOutputDim(
    conv_desc, in_desc, ker_desc, n, dims
  ));
  out_size = MakeStride(n, dims, strides);

  RequestMemory(&out_data, Context(DevType::kGPU(), 0), (out_size * dlt0->dtype.bits + 7) / 8);
  CUDNN_CALL(cudnnSetTensorNdDescriptor(out_desc, dt, n, dims, strides));

  algo = CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD;
  CUDNN_CALL(cudnnGetConvolutionForwardWorkspaceSize(handle, in_desc, ker_desc, conv_desc, out_desc, algo, &ws_size));
  RequestWorkspace(&ws, Context(DevType::kGPU(), 0), ws_size);
...
MNM_REGISTER_OP_DISPATCH("mnm.conv2d", DevType::kGPU(), "test_cudnn", Conv2dCUDNN::make);
