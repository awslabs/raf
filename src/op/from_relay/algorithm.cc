...
RAF_OP_FROM_RELAY("topk", "raf.op.topk",
                  [&](const Attrs& attrs, const Array<Expr>& args, const VarValueMap& val_map) {
                    ...
                    raf_args.push_back(MakeConstant(
                        IntValue::make(DataType::Int(64), relay_attrs->k.value().IntValue())));
                    raf_args.push_back(
                        MakeConstant(IntValue::make(DataType::Int(64), relay_attrs->axis)));
