...
OpInfo BatchFlattenMakeOutput(const Array<Value>& values, const Attrs& attrs) {
  ...
  if (common::shape_utils::IsCompact(*data.operator->())) {
    const int64_t* dshape = data->shape;
    int64_t flat{1};
    for (int i = 1; i < ndim; ++i) {
      flat = flat * int64_t{dshape[i]};
    }
    TensorValue output = TensorValue::make(data.CreateView({dshape[0], flat}, {}, nullptr));
    return OpInfo::make(output, data->ctx, false);
  }
  LOG(FATAL) << "ValueError: Only compact tensors are supported for now.";
  throw;
}
