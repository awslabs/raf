 ...
 * See: https://github.com/pytorch/pytorch/blob/master/c10/util/Half.h
 */

/*!
 * \file src/op/dialect/cuda/kernels/Half.h
 * \brief Defines the Half type (half-precision floating-point) including
 * conversions to standard C types and basic arithmetic operations. Note
 * that arithmetic operations are implemented by converting to floating
 * point and performing the operation in float32, instead of using CUDA
 * half intrinsics.
 */
#pragma once
...
inline float fp16_ieee_to_fp32_value(uint16_t h) {
   ...
   * constructud single-precision number by 2**(-24), i.e. the same amount.
   *
...
template <typename To, typename From>
typename std::enable_if<std::is_integral<From>::value && !std::is_same<From, bool>::value,
                        bool>::type
overflows(From f) {
