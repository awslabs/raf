 ...
 * See: https://github.com/pytorch/pytorch/blob/master/c10/macros/Macros.h
 */

/*!
 * \file src/impl/base_ops.cc
 * \brief Macros for Half
 */
...
// Suppose you were planning to write __launch_bounds__(a, b), based on your performance tuning on a
// modern GPU. Instead, you should write __launch_bounds__(C10_MAX_THREADS_PER_BLOCK(a),
// C10_MIN_BLOCKS_PER_SM(a, b)), which will also properly respect limits on old architectures.
...
#define CUDA_KERNEL_ASSERT(cond)                                                   \
  if (C10_UNLIKELY(!(cond))) {                                                     \
    __assert_fail(#cond, __FILE__, static_cast<unsigned int>(__LINE__), __func__); \
  }
#endif  // __APPLE__
