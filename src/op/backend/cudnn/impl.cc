...
MNM_OP_DISPATCH("mnm.op.avg_pool2d", AvgPool2DImplementedByCUDNNPoolingForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.avg_pool2d_dx", AvgPool2DDxImplementedByCUDNNPoolingBackward,
...
MNM_OP_DISPATCH("mnm.op.batch_norm_train",
                BatchNormTrainImplementedByCUDNNBatchNormalizationForwardTraining, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.batch_norm_train_dxwb",
                BatchNormTrainDxwbImplementedByCUDNNBatchNormalizationBackward, DevType::kCUDA(),
                "generated_cudnn");
class BiasAddImplementedByCUDNNAddTensor : public mnm::op::OpEnv {
  cudnnTensorDescriptor_t aDesc;
  cudnnTensorDescriptor_t cDesc;
  explicit BiasAddImplementedByCUDNNAddTensor(const CallValues& cv) {
    auto args = cv->args.as<mnm::op::schema::BiasAddArgs>();
    (void)args;
    DLTensor* b = args->b;
    (void)b;
    DLTensor* out = cv->out;
    (void)out;
    int axis = (args->axis + out->ndim) % out->ndim;
    std::vector<int> shape_aDesc;
    int prod_aDesc_1 = 1;
    for (int i = 0; i < 0 && i < b->ndim; ++i) {
      prod_aDesc_1 *= b->shape[i];
    }
    shape_aDesc.push_back(prod_aDesc_1);
    int prod_aDesc_2 = 1;
    for (int i = 0; i < b->ndim && i < b->ndim; ++i) {
      prod_aDesc_2 *= b->shape[i];
    }
    shape_aDesc.push_back(prod_aDesc_2);
    std::vector<int> padded_aDesc(PadDims<int, int>(shape_aDesc, 4));
    std::vector<int> stride_aDesc(Shape2Strides<int, int>(padded_aDesc));
    CUDNN_CALL(cudnnCreateTensorDescriptor(&aDesc));
    CUDNN_CALL(cudnnSetTensorNdDescriptor(aDesc, CUDNNDType(b->dtype),
                                          static_cast<int>(padded_aDesc.size()),
                                          BeginPtr(padded_aDesc), BeginPtr(stride_aDesc)));
    std::vector<int> shape_cDesc;
    int prod_cDesc_1 = 1;
    for (int i = 0; i < axis && i < out->ndim; ++i) {
      prod_cDesc_1 *= out->shape[i];
    }
    shape_cDesc.push_back(prod_cDesc_1);
    int prod_cDesc_2 = 1;
    for (int i = axis; i < axis + 1 && i < out->ndim; ++i) {
      prod_cDesc_2 *= out->shape[i];
    }
    shape_cDesc.push_back(prod_cDesc_2);
    int prod_cDesc_3 = 1;
    for (int i = axis + 1; i < out->ndim && i < out->ndim; ++i) {
      prod_cDesc_3 *= out->shape[i];
    }
    shape_cDesc.push_back(prod_cDesc_3);
    std::vector<int> padded_cDesc(PadDims<int, int>(shape_cDesc, 4));
    std::vector<int> stride_cDesc(Shape2Strides<int, int>(padded_cDesc));
    CUDNN_CALL(cudnnCreateTensorDescriptor(&cDesc));
    CUDNN_CALL(cudnnSetTensorNdDescriptor(cDesc, CUDNNDType(out->dtype),
                                          static_cast<int>(padded_cDesc.size()),
                                          BeginPtr(padded_cDesc), BeginPtr(stride_cDesc)));
    auto bytes_of_out = BytesCompactTensor(*out);
    RequestMemory(&out->data, cv->ctx, bytes_of_out);
  }

 public:
  ~BiasAddImplementedByCUDNNAddTensor() {
  ...
  void Execute(const CallValues& cv) {
    ...
    CUDNN_CALL(cudnnAddTensor(CUDNNThreadEntry::ThreadLocal()->handle,
                              CUDNNDType(out->dtype).const_addr<1>(), aDesc, b->data,
                              CUDNNDType(out->dtype).const_addr<1>(), cDesc, out->data));
  }
  static OpEnv* make(const CallValues& cv) {
    return new BiasAddImplementedByCUDNNAddTensor(cv);
  }
};
MNM_OP_DISPATCH("mnm.op.bias_add", BiasAddImplementedByCUDNNAddTensor, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.conv2d", Conv2DImplementedByCUDNNConvolutionForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.conv2d_dw", Conv2DDwImplementedByCUDNNConvolutionBackwardFilter,
                DevType::kCUDA(), "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.conv2d_dx", Conv2DDxImplementedByCUDNNConvolutionBackwardData,
                DevType::kCUDA(), "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.log_softmax", LogSoftmaxImplementedByCUDNNSoftmaxForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.log_softmax_dx", LogSoftmaxDxImplementedByCUDNNSoftmaxBackward,
                DevType::kCUDA(), "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.max_pool2d", MaxPool2DImplementedByCUDNNPoolingForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.max_pool2d_dx", MaxPool2DDxImplementedByCUDNNPoolingBackward,
                DevType::kCUDA(), "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.relu", ReluImplementedByCUDNNActivationForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.relu_dx", ReluDxImplementedByCUDNNActivationBackward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.sigmoid", SigmoidImplementedByCUDNNActivationForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.sigmoid_dx", SigmoidDxImplementedByCUDNNActivationBackward,
                DevType::kCUDA(), "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.softmax", SoftmaxImplementedByCUDNNSoftmaxForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.softmax_dx", SoftmaxDxImplementedByCUDNNSoftmaxBackward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.tanh", TanhImplementedByCUDNNActivationForward, DevType::kCUDA(),
                "generated_cudnn");
...
MNM_OP_DISPATCH("mnm.op.tanh_dx", TanhDxImplementedByCUDNNActivationBackward, DevType::kCUDA(),
                "generated_cudnn");
}  // namespace generated
}  // namespace cudnn
}  // namespace backend
