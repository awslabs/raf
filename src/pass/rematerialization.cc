...
class Rematerializer : public ExprMutator {
 ...
 private:
  ...
  Var CorrectType(LetList* scope, const Var& target_let_var) {
    static const auto reshape_op = Op::Get("mnm.op.reshape");
  ...
  Var Rematerialize(LetList* scope, const Var& var, size_t curr_depth = 1) {
    ...
    CHECK(call_node != nullptr) << "Tensor " << latest_let_var
                                ...
                                << mnm::ir::AsText(let_vars_[latest_let_var]);
...
Pass Rematerialization() {
  ...
  return MNMSequential({InferType(), func_pass, DeadCodeElimination()}, pass_info);
}

MNM_REGISTER_GLOBAL("mnm.pass_.Rematerialization").set_body_typed(Rematerialization);
