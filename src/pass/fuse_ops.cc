...
class IndexedForwardGraph::Creator : private ExprVisitor {
 ...
 private:
  ...
  /*! \brief The let binding map from variable to the value expr. */
  std::unordered_map<Expr, Expr, ObjectPtrHash, ObjectPtrEqual> let_binding_;
  ...
  void VisitExpr_(const CallNode* call) final {
    ...
    for (size_t i = 0; i < call->args.size(); ++i) {
      ...
      this->Update(arg, node, edge_pattern);
    }
