...
def test_basic():
    ...
    def expected(shape):
        ...
        let5 = relay.Let(ret, relay.Tuple([a1, gradient]), ret)
        let4 = relay.Let(gradient, relay.Tuple([x1, x2]), let5)
        v2 = relay.Call(get_reduce_axis_op, [dy, y])
        v3 = relay.Call(get_kept_dims_op, [dy, y])
        let3 = relay.Let(x2, relay.Call(sum_op, [dy, v2, v3]), let4)
        v0 = relay.Call(get_reduce_axis_op, [dy, x])
        v1 = relay.Call(get_kept_dims_op, [dy, x])
        let2 = relay.Let(x1, relay.Call(sum_op, [dy, v0, v1]), let3)
