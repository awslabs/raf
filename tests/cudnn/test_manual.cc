...
using mnm::DType;
using mnm::DTypeCode;
using mnm::op::attrs::Conv2DAttrs;

TEST(CuDNN, RegisterConvCudnn) {
  const auto *dispatch_list = OpDispatch::Get("mnm.conv2d", DevType::kGPU());
  ASSERT_EQ(dispatch_list->size(), 1);

  Context ctx(DevType::kGPU(), 0);
  ...
  Attrs attrs = Conv2DAttrs::make({1, 1}, {0, 0}, {1, 1}, 1);
